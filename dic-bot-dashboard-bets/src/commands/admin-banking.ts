...
export const command = {
  ...
  data: new SlashCommandBuilder()
    .setName("adminbank")
    .setDescription("Bank & lines admin")
    .addSubcommand(sc => sc.setName("grant").setDescription("Grant DIC$ to a coach")
      .addUserOption(o => o.setName("coach").setDescription("Coach").setRequired(true))
      .addIntegerOption(o => o.setName("amount").setDescription("Amount").setRequired(true)))
    .addSubcommand(sc => sc.setName("set").setDescription("Set DIC$ balance")
      .addUserOption(o => o.setName("coach").setDescription("Coach").setRequired(true))
      .addIntegerOption(o => o.setName("balance").setDescription("New balance").setRequired(true)))
    .addSubcommand(sc => sc.setName("reset").setDescription("Reset all wallets to start balance for new season")
      .addIntegerOption(o => o.setName("season").setDescription("Season number").setRequired(true))
      .addIntegerOption(o => o.setName("start").setDescription("Starting DIC$ per coach").setRequired(true)))
    .addSubcommand(sc => sc.setName("linessync").setDescription("Sync betting lines from Google Sheet")),
    ...
    if (sub === "reset") {
      ...
      for (const c of coaches) {
        const w = await prisma.wallet.upsert({
          where: { coachId: c.id },
          update: { balance: start, season },
          create: { coachId: c.id, balance: start, season }
        });
        n++;
      }
      await interaction.reply(`Reset ${n} wallets to DIC$ ${start} for Season ${season}.`);
      return;
    }
    ...
    if (sub === "linessync") {
      ...
      for (const r of rows) {
        ...
        await prisma.line.create({
          data: { season, week, homeTeam, awayTeam, spread: spread as any, total: total as any, homeML: homeML as any, awayML: awayML as any, cutoff: cutoff as any }
        });
        count++;
      }
      await interaction.reply(`Imported ${count} lines from Google Sheet (Lines tab).`);
      return;
    }
  }
} as const;
