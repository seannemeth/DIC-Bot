...
async function resolveGame(season:number, week:number, home:string, away:string) {
  // Try to match by team names in Game table (confirmed or pending in same week/season)
  ...
  const game = await prisma.game.findFirst({ where: { season, week, homeCoachId: homeCoach.id, awayCoachId: awayCoach.id } });
  return game;
}
...
export const command = {
  data: new SlashCommandBuilder()
    .setName("bet")
    .setDescription("Place a bet (spread, ml, total) using Google Sheet lines")
    .addStringOption(o => o.setName("market").setDescription("spread|ml|total").setRequired(true))
    .addStringOption(o => o.setName("home").setDescription("Home team (exact as sheet)").setRequired(true))
    .addStringOption(o => o.setName("away").setDescription("Away team (exact as sheet)").setRequired(true))
    ...
    .addIntegerOption(o => o.setName("amount").setDescription("Stake in DIC$").setRequired(true)),
  async execute(interaction:any) {
    ...
    if (!line) { await interaction.reply({ content: "No line found for that game/season/week. Ask an admin to run /adminbank linessync.", ephemeral:true }); return; }
    ...
    let storedLine: number | null = null;
    ...
    const bet = await prisma.bet.create({
      data: {
        coachId: me.id, season, week, gameId: game?.id || null, market, side, line: storedLine as any, price: price as any, amount, snapshotId: snap.id
      }
    });
