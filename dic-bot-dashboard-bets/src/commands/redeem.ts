...
export const command = {
  data: new SlashCommandBuilder()
    .setName("redeem")
    .setDescription("Redeem DIC$ for an attribute upgrade voucher")
    .addStringOption(o => o.setName("item").setDescription("Upgrade ID (e.g., ATTR+1)").setRequired(true)),
  async execute(interaction:any) {
    ...
    const emb = new EmbedBuilder().setTitle("Upgrade Voucher Created")
      .setDescription(`${me.team || me.handle} redeemed **${item.name}** for DIC$ ${item.cost}\nRedemption #${red.id}`)
      .setColor(0x8e44ad);
