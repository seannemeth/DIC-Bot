...
async function loadConferences() {
  const res = await fetch('/api/conferences'); const list = await res.json();
  const sel = qs('#conf-select'); sel.innerHTML='';
  list.forEach(c => { const o = document.createElement('option'); o.value=c; o.textContent=c; sel.appendChild(o); });
}
async function loadConfStandings() {
  const conf = qs('#conf-select').value;
  const res = await fetch('/api/standings?type=overall&conference='+encodeURIComponent(conf));
  const data = await res.json();
  const rows = data.rows || data; 
  const tbody = qs('#conf-standings tbody'); tbody.innerHTML='';
  rows.forEach((r,i)=>{
    const tr = document.createElement('tr');
    tr.innerHTML = `<td>${i+1}</td><td>${r.team}</td><td>${r.w}-${r.l}${r.t?'-'+r.t:''}</td><td>${r.pf}</td><td>${r.pa}</td><td>${r.diff}</td>`;
    tbody.appendChild(tr);
...
qs('#tab-conferences').addEventListener('click', ()=>{ switchTab('conferences'); loadConferences().then(loadConfStandings); });
...
qs('#conf-load').addEventListener('click', ()=> loadConfStandings());
qs('#load-lines').addEventListener('click', ()=> loadLines());
qs('#copy-bet').addEventListener('click', async ()=>{ const txt = qs('#bet-command').textContent; try{ await navigator.clipboard.writeText(txt);}catch{} });
