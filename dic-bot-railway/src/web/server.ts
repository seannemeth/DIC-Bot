...
export async function startWebServer(prisma: PrismaClient) {
  ...
  app.get("/api/coaches", async (_req, res) => {
    try { res.json(await prisma.coach.findMany()); }
    catch(e:any){ res.status(500).json({ error: e.message }); }
  });

  app.get("/api/h2h", async (req, res) => {
    try {
      const aId = Number(req.query.a), bId = Number(req.query.b);
      if (!aId || !bId) { res.status(400).json({ error: "Params a,b required" }); return; }
      const games = await prisma.game.findMany({ where: { status:"confirmed",
        OR: [{ homeCoachId:aId, awayCoachId:bId }, { homeCoachId:bId, awayCoachId:aId }] }, orderBy:{ playedAt:"desc" } });
      ...
      for (const g of games) {
        ...
        margins.push((aPts||0)-(bPts||0));
        if ((aPts||0)>(bPts||0)) aW++; else if ((aPts||0)<(bPts||0)) bW++; else ties++;
      }
  ...
  app.listen(PORT, () => { console.log(`DIC dashboard: http://localhost:${PORT}`); });
}
