...
export async function settleBetsForGame(prisma: PrismaClient, gameId: number) {
  ...
  for (const b of bets) {
    ...
    if (b.market === "ml") {
      ...
      result = homeWon === pickHome ? "won" : "lost";
    } else if (b.market === "spread" && b.line != null) {
      const margin = game.homePts - game.awayPts;
      const coverHome = margin > b.line;
      const coverAway = -margin > -b.line;
      if (b.side === "home") {
    ...
    let payout = 0;
    if (result === "won") {
      // default -110 for spread/total; ML uses price if provided
      if (b.market === "ml" && b.price) {
        payout = b.price > 0 ? Math.round(b.amount + (b.amount * b.price / 100)) : Math.round(b.amount + (b.amount * 100 / Math.abs(b.price)));
      } else {
        payout = Math.round(b.amount + (b.amount * 100 / 110));
      }
    } else if (result === "push") {
      payout = b.amount;
    }
    ...
    if (payout > 0) {
      await prisma.wallet.update({ where: { coachId: b.coachId }, data: { balance: { increment: payout } } });
    }
  }
}
