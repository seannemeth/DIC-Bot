...
export const command = { data: new SlashCommandBuilder().setName("leaderboard").setDescription("Top DIC$ balances"),
  async execute(interaction:any){
    const wallets = await prisma.wallet.findMany({ orderBy:{ balance:"desc" }, take:10, include:{ coach:true } });
    const lines = wallets.map((w: any, i: number) => `**${i+1}. ${w.coach.team || w.coach.handle}** — DIC$ ${w.balance}`);
