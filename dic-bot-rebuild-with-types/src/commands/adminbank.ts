...
export const command = {
  ...
  data: new SlashCommandBuilder().setName("adminbank").setDescription("Bank & lines admin")
    ...
    .addSubcommand(sc=>sc.setName("reset").setDescription("Reset all wallets for new season").addIntegerOption(o=>o.setName("season").setDescription("Season").setRequired(true)).addIntegerOption(o=>o.setName("start").setDescription("Start DIC$").setRequired(true)))
    .addSubcommand(sc=>sc.setName("linessync").setDescription("Import betting lines from Google Sheet tab 'Lines'")),
  async execute(interaction:any){
    ...
    if (sub==="grant"){
      ...
      const w = await prisma.wallet.upsert({ where:{ coachId: coach.id }, update:{ balance:{ increment: amt } }, create:{ coachId: coach.id, balance: amt } });
      await interaction.reply(`Granted DIC$ ${amt} to ${coach.team || coach.handle}. New balance: ${w.balance}`);
      return;
    }
    if (sub==="reset"){
      ...
      for (const c of coaches){
        await prisma.wallet.upsert({ where:{ coachId:c.id }, update:{ balance:start, season }, create:{ coachId:c.id, balance:start, season } });
        n++;
      }
      await interaction.reply(`Reset ${n} wallets to DIC$ ${start} for Season ${season}.`); return;
    }
    if (sub==="linessync"){
      ...
      await interaction.reply(`Imported ${count} lines from Google Sheet (Lines tab).`); return;
    }
  }
} as const;
