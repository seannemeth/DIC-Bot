...
export async function computePower(prisma = new PrismaClient()): Promise<PowerRow[]> {
  ...
  for (const g of games) { byCoach[g.homeCoachId].push(g.awayCoachId); byCoach[g.awayCoachId].push(g.homeCoachId); }
  const sos: Record<number, number> = {};
  for (const c of coaches as any[]) {
    const opps = byCoach[c.id];
    sos[c.id] = opps.length ? opps.reduce((s:number,oid:number)=>s+(elo[oid]||1500),0)/opps.length : 1500;
  }
  const form: Record<number, number> = {}; for (const c of coaches as any[]) form[c.id]=0;
  const rows: PowerRow[] = (coaches as any[]).map((c)=>({ coachId:c.id, team:c.team||c.handle, elo:elo[c.id], sos:sos[c.id], form:form[c.id], composite:0 }));
