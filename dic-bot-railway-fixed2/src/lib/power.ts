...
export async function computePower(prisma = new PrismaClient()): Promise<PowerRow[]> {
  ...
  const byCoach: Record<number, Game[]> = {}; coaches.forEach((c: any)=>byCoach[c.id]=[]);
  ...
  rows.forEach((r: any, i: number)=>{ r.composite = Math.round(eloN[i] + 0.35*sosN[i] + formN[i]); });
  const compN = norm(rows.map(r=>r.composite)); rows.forEach((r: any, i: number)=> r.composite = Math.round(compN[i]));
